    ...
    public partial class TextRenderingDispatchSystem : SystemBase
    {
        ...
        protected override void OnCreate()
        {
            ...
            m_glyphsAndMasksQuery = SystemAPI.QueryBuilder()
                                    .WithAll<RenderGlyphOld, TextRenderControl, RenderBounds>()
                                    .WithAll<TextShaderIndexOld, TextMaterialMaskShaderIndex, RenderGlyphMask>()
        ...
        JobHandle UpdateRenderGlyphChunks(JobHandle inputDependencies, Entity textStatisticsSingleton)
        {
            ...
            var collectGlyphsJh        = new GatherGlyphUploadOperationsJobChunk
            {
                ...
                textShaderIndexHandle     = SystemAPI.GetComponentTypeHandle<TextShaderIndexOld>(false),
            ...
            if (textGpuUploadOperationsCount != 0)
            {
                ...
                m_TextThreadedGPUUploader = StartBufferUpdate(ref m_GPUTextBuffer,
                                                              m_textBufferID,
                                                              m_TextGPUBufferSizeMax,
                                                              ref m_TextGPUUploader,
                                                              ref m_TextGPUAllocator,
                                                              ref m_TextCurrentDataSize,
                                                              textTotalUploadBytes.Value,
                                                              textBiggestUploadBytes.Value,
                                                              textGpuUploadOperationsCount);
        ...
        JobHandle UpdateMaskChunks(JobHandle inputDependencies, Entity textStatisticsSingleton)
        {
            ...
            var copyPropertiesJh = new CopyGlyphShaderIndicesJob
            {
                ...
                textShaderIndexLookup = SystemAPI.GetComponentLookup<TextShaderIndexOld>(false)
            ...
            if (numGpuMaskUploadOperations != 0)
            {
                ...
                m_MaskThreadedGPUUploader = StartBufferUpdate(ref m_GPUMaskBuffer,
                                                              m_MaskBufferID,
                                                              m_MaskGPUBufferSizeMax,
                                                              ref m_MaskGPUUploader,
                                                              ref m_MaskGPUAllocator,
                                                              ref m_MaskCurrentDataSize,
                                                              totalMaskUploadBytes.Value,
                                                              biggestMaskUploadBytes.Value,
                                                              numGpuMaskUploadOperations);
            ...
            int uploadOperationsCount)
        {
            ...
            if (persistentBytes > currentBufferBytes)
            {
                ...
                if (persistentBytes > targetBufferMaxSize)
                    Debug.LogError(
                        "TextMeshDOTS: The text or mask GPU buffer needs more than 1GiB of persistent GPU memory. This is more than some GPU backends can allocate. Try to reduce amount of loaded data.");
