...
public unsafe class ProceduralAll : MonoBehaviour
{
    public Mesh mesh;
    ...
    public JobHandle OnPerformCulling(BatchRendererGroup rendererGroup, BatchCullingContext cullingContext, BatchCullingOutput cullingOutput, IntPtr userContext)
    {
        ...
        uint _gpuIndexedIndirectBufferBaseOffset = 0;
        uint indirectDrawIndexedArgsSize = GraphicsBuffer.IndirectDrawIndexedArgs.size;
        ...
        for (uint i = 0; i < drawCallCount; ++i)
        {
            drawCommands.proceduralDrawCommands[i] = new BatchDrawCommandProcedural
            {
                ...
                sortingPosition = 0,
                visibleOffset = visibleBaseOffset + i * 2,
                visibleCount = 2,
        ...
        visibleBaseOffset += drawCallCount * visibleCount;
        ...
        for (uint i = 0; i < drawCallCount; ++i)
        {
            drawCommands.proceduralIndirectDrawCommands[i] = new BatchDrawCommandProceduralIndirect
            {
                ...
                indirectArgsBufferOffset = _gpuIndexedIndirectBufferBaseOffset + i * indirectDrawIndexedArgsSize,
            };
        }
        visibleBaseOffset += drawCallCount * visibleCount;
        _gpuIndexedIndirectBufferBaseOffset += drawCallCount * indirectDrawIndexedArgsSize;
        ...
        for (uint i = 0; i < drawCallCount; ++i)
        {
            drawCommands.proceduralIndirectDrawCommands[5 + i] = new BatchDrawCommandProceduralIndirect
            {
                ...
                indirectArgsBufferOffset = i * indirectDrawArgsSize,
            };
        }
        visibleBaseOffset += drawCallCount * visibleCount;
